...
class Application:
    def __init__(self) -> None:
        ...
        self.perf_monitor.set_extra_snapshot_provider(self._collect_perf_snapshot_context)
    ...
    def _collect_perf_snapshot_context(self) -> dict:
        state = self.time_controller.get_state()
        return {
            "runtime": {
                "real_dt_ms": round(self.last_real_dt * 1000.0, 4),
                "fps_target": FPS,
                "fps_estimated": round((1.0 / self.last_real_dt), 3) if self.last_real_dt > 0 else 0.0,
                "simulation_speed": state.current_speed(),
                "simulation_paused": state.is_paused,
            },
            "camera": {
            ...
            "world": self.world.collect_diagnostics(),
        }
